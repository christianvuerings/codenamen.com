...
socket.on("serverStats", (data) => {
  ...
  document.getElementById("server-stats").innerHTML =
    "Spelers: " + data.players + " | Kamers: " + data.rooms;
});
...
function updateInfo(game, team) {
  ...
  const teamNaam = game.turn === "blue" ? "Blauw" : "Rood";

  turnMessage.innerHTML = `${teamNaam} 's beurt`; // Update the turn msg
  turnMessage.className = game.turn; // Change color of turn msg
  if (game.over) {
    // Display winner
    turnMessage.innerHTML = game.winner + " wint!";
...
function updateTimerSlider(game, mode) {
  ...
  timerSliderLabel.innerHTML = "Timer Duur : " + timerSlider.value + "min";
...
function updatePacks(game) {
  ...
  document.getElementById("word-pool").innerHTML =
    "Aantal Woorden: " + game.words.length;
}
